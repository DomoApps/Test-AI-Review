...
<p align="center">
   <img src="../../../../assets/images/asset-library.png" width="500">
</p>
...
const query = domo.get('/data/v1/dataset').then(
res => prepareData(res));

function prepareData(data) {
    const labels = [...new Set(data.map(
item => item['Date']))];
    const stacks = [...new Set(data.map(
item => item['Stacks']))];
    const categories = Object.keys(data[0]).filter(
key => key.startsWith('Bar'));
...
function createDatasets(data, labels, stacks, categories) {
    ...
    const datasets = categories.flatMap(
category => 
        stacks.map(
stack => ({
            label: stack,
...
function calculateTrendedLine(data, labels, datasets) {
    const trendedPercentageData = labels.map(
label => {
        const billableHours = data.filter(
item => item['Date'] === label).reduce((sum, item) => sum + (item['Bar1'] || 0), 0);
        const extra1 = data.filter(
item => item['Date'] === label).reduce((sum, item) => sum + item['Extra1'], 0);
        const extra2 = data.filter(
item => item['Date'] === label).reduce((sum, item) => sum + item['Extra2'], 0);
                    ...
                    labels: {
                        value: {
                            ...
                            formatter: (value, context) => 
                                context.dataset.label === 'Trend %' ? `${value.toFixed(1)}%` : value > 0 ? value : ''
                        },
                        legend: {
                            display: true,
                            color: "black",
...
ds => ds.stack === stack).indexOf(dataset) === 0 && stack !== undefined ? stackLabels[stack] : '';
                            }
                        }
                    },
                },
                legend: {
                    ...
                    labels: {
                        ...
                        boxHeight: 4,
                        boxWidth: 8,
                        generateLabels: () => stacks.map((item, datasetIndex) => ({ text: item, fillStyle: stackColors[item] })),
                    },
                    maxHeight: 300,
                },
            },
